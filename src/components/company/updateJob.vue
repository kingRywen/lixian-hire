<template>
  <div>
    <md-stepper ref="step" @completed="completed()">
      ...
      <md-step md-label="更新职位" :md-error="!validJob" :md-continue="validJob" :md-message="invalidJobMessage" md-button-back="返回">
        <p>以下信息更新你的职位</p>
...
export default {
  ...
  methods: {
    ...
    completed () {
      ...
      self.$http.post('/auth/update-position', {
        name, salary, location: jobLocation, experience, education, content, code: this.$route.params.id
      })
        .then((res) => {
          if (res.data.success) {
            console.log('更新职位成功')
